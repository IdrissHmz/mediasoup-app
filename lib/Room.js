...
//const  = require('@sitespeed.io/throttle');
...
class Room extends EventEmitter {
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
		...
		// peer.data.remotePorts = new Map();
		peer.data.remotePorts = new Array();
		...
		peer.on('close', () => {
			...
			// // Killing the process of the peer when closing it in Room.js. Gstreamer/FFmpeg
			// if (peer && peer.data.process) {
			// 	peer.data.process.kill();
			// 	peer.data.process = undefined;
			// }
	...
	async _handleProtooRequest(peer, request, accept, reject) {
		switch (request.method) {
			...
			case 'start-record':
				{
					// RECORD
					const { peerId, roomId } = request.data;
					console.log('handleStartRecordRequest() [data:%o]', peerId);
					console.log(peer.data.joined)
					// const peer = this._getJoinedPeers().get(roomId);
					// peer = peer;
			...
			case 'stop-record':
				{
					const { peerId, roomId } = request.data;
					console.log('handleStopRecordRequest() [data:%o]', peerId);
					// const peer = this._getJoinedPeers().get(roomId);
					...
					if (!peer.data.process) {
						throw new Error(`Peer with id ${peerId} is not recording`);
					}
					...
					for (const remotePort of peer.data.remotePorts) {
	...
	_createTransport = async (transportType, options) => {
		console.log('createTransport() [type:%s. options:%o]', transportType, options);

		switch (transportType) {
			case 'webRtc':
				return await this._mediasoupRouter.createWebRtcTransport(config.mediasoup.webRtcTransport);
			case 'plain':
				return await this._mediasoupRouter.createPlainTransport(config.mediasoup.plainRtpTransport);
		}
	};

	_publishProducerRtpStream = async (peer, producer, ffmpegRtpCapabilities) => {
		console.log('publishProducerRtpStream()');
		console.log(producer.id);

		// Create the mediasoup RTP Transport used to send media to the GStreamer process
		const rtpTransportConfig = config.mediasoup.plainRtpTransport;
		// const rtpTransportConfig = config.plainTransportOptions;
		...
		const rtpTransport = await this._createTransport('plain', rtpTransportConfig);
	...
	_startRecord = async (peer) => {
		let recordInfo = {};
		console.log('length producers');
		console.log(peer.data.producers.size);
		for (const [producerId, producer] of peer.data.producers) {
			recordInfo[producer.kind] = await this._publishProducerRtpStream(peer, producer);
			// console.log(recordInfo[producer.kind])
		}

		recordInfo.fileName = Date.now().toString();

		peer.data.process = this._getProcess(recordInfo);

		setTimeout(async () => {
			for (const [consumerId, consumer] of peer.data.consumers) {
