...
class Room extends EventEmitter {
	...
	async _handleProtooRequest(peer, request, accept, reject) {
		switch (request.method) {
			...
			case 'start-record':
				{
					...
					console.log('handleStartRecordRequest() [data:%o]', roomId);
					// const peer = this._getJoinedPeers().get(roomId);
					const peer = peer;

					if (!peer) {
						throw new Error(`Peer with id ${roomId} was not found`);
					}
			...
			case 'stop-record':
				{
					const { roomId } = request.data;
					console.log('handleStopRecordRequest() [data:%o]', roomId);
					// const peer = this._getJoinedPeers().get(roomId);
					const peer = peer;
					...
					if (!peer.process) {
						throw new Error(`Peer with id ${roomId} is not recording`);
					}
	...
	_createTransport = async (transportType, options) => {
		...
		switch (transportType) {
			case 'webRtc':
				return await this._mediasoupRouter.createWebRtcTransport(config.webRtcTransport);
			case 'plain':
				return await this._mediasoupRouter.createPlainRtpTransport(config.plainRtpTransport);
		}
	};
	...
	_publishProducerRtpStream = async (peer, producer, ffmpegRtpCapabilities) => {
		...
		const rtpTransport = await _createTransport('plain', rtpTransportConfig);
	...
	_startRecord = async (peer) => {
		...
		for (const producer of peer.producers) {
			recordInfo[producer.kind] = await _publishProducerRtpStream(peer, producer);
		}

		recordInfo.fileName = Date.now().toString();

		peer.process = _getProcess(recordInfo);
